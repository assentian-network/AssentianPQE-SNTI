...
bool WalletBatch::WriteXmssState(const std::vector<uint8_t>& state)
{
    ...
    return WriteIC(std::string("xms"), value);
}

bool WalletBatch::ReadXmssState(std::vector<uint8_t>& state)
{
    std::string value;
    if (!m_batch->Read(std::string("xms"), value)) return false;
    state.assign(value.begin(), value.end());
    return true;
}

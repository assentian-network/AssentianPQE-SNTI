...
IsMineResult IsMineInner(const LegacyScriptPubKeyMan& keystore, const CScript& scriptPubKey, IsMineSigVersion sigversion, bool recurse_scripthash=true)
{
    ...
    case TxoutType::P2XMSS:
    {
        ...
        if (vSolutions[0].size() != 64) break;
        // Hash the pubkey and check against wallet's XMSS keys
        uint160 addr_hash = XMSSAddr::Hash(vSolutions[0]);
...
{
    LOCK(cs_KeyStore);
    return m_xmss_keys.count(addr_hash) > 0;
}

bool LegacyScriptPubKeyMan::AddXMSSKey(const std::vector<uint8_t>& pubkey, const std::vector<uint8_t>& seckey)
{
    if (pubkey.size() != 64) return false;
    LOCK(cs_KeyStore);
    uint160 addr_hash = XMSSAddr::Hash(pubkey);
    m_xmss_keys[addr_hash] = pubkey;
    // TODO: encrypt and persist seckey to CWalletDB
    return true;
}

bool LegacyScriptPubKeyMan::GetXMSSPubKey(const uint160& addr_hash, std::vector<uint8_t>& pubkey_out) const
{
    LOCK(cs_KeyStore);
    auto it = m_xmss_keys.find(addr_hash);
    if (it == m_xmss_keys.end()) return false;

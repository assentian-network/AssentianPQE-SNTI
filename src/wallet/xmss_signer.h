...
private:
    ...
    struct XMSSKeyEntry {
        ...
        std::string label;
        // mutable: SignXMSS() is const (required to override
        // SigningProvider::SignXMSS, which every real signing call site
        // invokes through a `const SigningProvider&`) but must still
        // advance leaf_index/retired under cs_xmss_signer -- same
        // logical-constness pattern as the mutex itself, instead of the
        // const_cast this replaces (audit H-03, 31 Jul 2026).
        mutable uint32_t leaf_index{0};

...
static bool CreateXMSSSig(const BaseSignatureCreator& creator, SignatureData& sigdata, const SigningProvider& provider, std::vector<unsigned char>& sig_out, const std::vector<unsigned char>& pubkey, const CScript& scriptcode, SigVersion sigversion)
{
    ...
    auto it = sigdata.xmss_signatures.find(pubkey);
    if (it != sigdata.xmss_signatures.end()) {
        sig_out = it->second;
        return true;
    }

    // Compute sighash using the creator's transaction data
    uint256 sighash;
    try {
        const auto& mtx_creator = dynamic_cast<const MutableTransactionSignatureCreator&>(creator);
        sighash = SignatureHash(scriptcode, mtx_creator.GetTxTo(), mtx_creator.GetNIn(), SIGHASH_ALL, mtx_creator.GetAmount(), sigversion, mtx_creator.GetTxData());
